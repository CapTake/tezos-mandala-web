    ...
    <v-row v-if="paged.length" class="py-5" style="max-width: 100vw" :key="sort">
      <template v-for="item in paged">
        <v-col :key="item.id" md="4" sm="6" style="min-width: 290px">
          <mandala-card :item="item" :actions="true" show-id default-uri="/seed.png" @send="showSendDialog" @rename="showRenameDialog" @convert="renderV2" />
        </v-col>
      </template>
...
export default {
  ...
  data: () => ({
    ...
    sort: 'New',
    sortItems: ['Id', 'Price', 'New', 'Name', 'Rarity'],
    selectedToken: null,
    sendDialog: false,
    renameDialog: false
  }),
  ...
  methods: {
    ...mapActions(['connectWallet', 'renderMandala', 'transferToken', 'renameMandala', 'setBuyStatus', 'signWithMintKey']),
    async loadGraphData () {
      ...
      try {
        ...
        const ids = this.paged.filter((_, i) => i >= (page * pageSize) && i < (page + 1) * pageSize).map(({ id }) => id)
        await this.$store.dispatch('readMandalasData', ids)
        this.$forceUpdate()
      } catch (e) {
        console.log(e)
      } finally {
    ...
    async renderV2 (id) {
      await this.renderMandala(id)
    },
    async renderV1 ({ id, bytes }) {
      const param = {
        id,
        ...await this.signWithMintKey(bytes)
      }
      console.log(param)
      this.renderV1Mandala(param)
    },
    showSendDialog (item) {
  ...
  computed: {
    ...mapGetters(['getFiltered', 'isConnected']),
    ...
    comparator () {
      const comp = {
        ...
        Version: (a, b) => parseInt(a.version) - parseInt(b.version),
        Price: (a, b) => a.id - b.id,
    ...
    paged () {
      const res = this.listed.filter((e, i) => (this.page * this.pageSize + this.pageSize) > i)
      return res
    },
    listed () {
      const res = this.getFiltered(this)
      return res
    }
  }
}
